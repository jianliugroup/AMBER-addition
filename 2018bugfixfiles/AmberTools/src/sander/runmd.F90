...
#  endif
  ...
  ! Ek(t) is output in mden or mdout
  ! Ek(t+dt/2) is written in file_ek, as defined below
  integer,parameter :: fh_ek = 1024 ! a number that has not been used
...
#endif
  ! }}}
  if (ischeme > 0 .and. master .and. rem == 0) then
...
#endif /* LES */
  if ( ifbox==0 .and. is_langevin .and. ischeme == 0) then
...
!------------------------------------------------------------------------------
  ...
  if (icnste /= 0 .and. mdloop .eq. 0) call cnsteinit(x, ig)
...
!------------------------------------------------------------------------------
  ...
  if ((icnstph == 1) .and. on_cpstep .and. &
      .not. on_cestep) then
    call cnstphendstep(xx(l190), xx(l15), ener%pot%dvdl, temp0, solvph)
    if (master) call cnstphwrite(rem,remd_types,replica_indexes)
  end if
  ...
  if ((icnste == 1) .and. on_cestep) then
...
!------------------------------------------------------------------------------
  ...
  if (ivscm) then
    ...
    if (ifbox == 0) then
      if ((is_langevin .and. ischeme==0) .or. (ischeme>0.and.ipimd==0)) then
...
!------------------------------------------------------------------------------
    ...
    if (ntwe > 0 .and. mod(total_nstep,ntwe) == 0 .and. onstep) then
...
!------------------------------------------------------------------------------
    ...
    if (ischeme > 0 .and. master .and. rem == 0) close(fh_ek)
